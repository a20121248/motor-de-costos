...
public class ListarControlador implements Initializable,ObjetoControladorInterfaz {
    ...
    public void initialize(URL url, ResourceBundle rb) {        
        ...
        // Tabla: Buscar
        List<CentroDriver> listaEntidades = centroDAO.listarCentrosObjetosConDriver(menuControlador.periodoSeleccionado,"-",menuControlador.repartoTipo,-1);
    ...
    @FXML void btnAsignarDriverObjetoAction(ActionEvent event) {        
        ...
        if (!entidadSeleccionada.getCodigoDriver().equals("Sin driver asignado")) {
            if (menuControlador.navegador.mensajeConfirmar("Asignar driver que distribuye a Objetos de Costos", "La entidad ya cuenta con un driver asignado.\n¿Está seguro que desea reemplazar dicho driver?")) {
                centroDriverDAO.borrarAsignacionDriverObjeto(entidadSeleccionada.getCodigoCentro(), entidadSeleccionada.getGrupoGasto().getCodigo(), menuControlador.periodoSeleccionado, menuControlador.repartoTipo);
                buscarDriverObjeto();
            }
            return;
        }
    ...
    @FXML void btnQuitarAction(ActionEvent event) {
        ...
        centroDriverDAO.borrarAsignacionDriverObjeto(entidadSeleccionada.getCodigoCentro(), entidadSeleccionada.getGrupoGasto().getCodigo(), menuControlador.periodoSeleccionado, menuControlador.repartoTipo);
        menuControlador.Log.deleteItemPeriodo(LOGGER, menuControlador.usuario.getUsername(), entidadSeleccionada.getCodigoDriver() + " de (" + entidadSeleccionada.getCodigoCentro()+ ")", menuControlador.periodoSeleccionado, menuControlador.navegador.RUTAS_DRIVER_ENTIDAD_CENTROS_OBJETOS_LISTAR.getDireccion());
        buscarPeriodo(menuControlador.periodoSeleccionado, cmbTipoCentro.getValue().getCodigo(), menuControlador.repartoTipo);
    }
    ...
    public void seleccionarDriverObjeto(DriverObjeto driver) {
        ...
        centroDriverDAO.asignarDriverObjeto(entidadSeleccionada.getCodigoCentro(),entidadSeleccionada.getGrupoGasto().getCodigo(), driver.getCodigo(), menuControlador.periodoSeleccionado, menuControlador.repartoTipo);
        menuControlador.Log.agregarItemPeriodo(LOGGER, menuControlador.usuario.getUsername(), driver.getCodigo() + " a (" + entidadSeleccionada.getCodigoCentro() + ")", menuControlador.periodoSeleccionado, menuControlador.navegador.RUTAS_DRIVER_ENTIDAD_CENTROS_OBJETOS_LISTAR.getDireccion());
        buscarPeriodo(menuControlador.periodoSeleccionado, cmbTipoCentro.getValue().getCodigo(), menuControlador.repartoTipo);
    }
